...
public class MetadataParser {
  ...
  public MetadataParser(BufferedReader bufferedReader) {
    this.bufferedReader = requireNonNull(bufferedReader);
  }

  @SuppressWarnings("UnstableApiUsage")
  public Stream<Record> stream() {
    return Streams.stream(new MetadataIterator(bufferedReader));
  }

  private static class MetadataIterator implements Iterator<Record> {
    private final BufferedReader bufferedReader;
    private Record record;

    private MetadataIterator(BufferedReader bufferedReader) {
      this.bufferedReader = requireNonNull(bufferedReader);
    }
